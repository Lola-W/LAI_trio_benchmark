...
def calculate_mendelian_violation_rate(intervals_file: str, pop_file: str, ped_file: str, output_file: str):
    print("Loading data...")
    try:
    ...
    if df_evaluable.empty:
        print("Warning: No fully evaluable rows found. Check your panel and pedigree files!")
        return
    df_evaluable['Is_Violation'] = df_evaluable['Is_Violation'].astype(bool)
    violations_df = df_evaluable[df_evaluable['Is_Violation']]
    
    total_lengths = df_evaluable.groupby('Child_ID')['BP_Length'].sum().rename('Total_Evaluable_Length')
    violation_lengths = violations_df.groupby('Child_ID')['BP_Length'].sum().reindex(total_lengths.index, fill_value=0).rename('Violation_Length')
    
    summary_df = pd.concat([total_lengths, violation_lengths], axis=1)
    summary_df['Violation_Rate'] = summary_df['Violation_Length'] / summary_df['Total_Evaluable_Length']
    summary_df.reset_index(inplace=True)
    
    summary_df.to_csv(output_file, sep='\t', index=False)
    print(f"Successfully processed {len(summary_df)} children.")
    print(f"Results saved to: {output_file}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Calculate Mendelian Violation Rate using Pedigree and Population mapping.")
    ...
    calculate_mendelian_violation_rate(args.intervals_file, args.pop_file, args.ped_file, args.output)